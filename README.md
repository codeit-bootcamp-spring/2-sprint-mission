...
- [x] 채널, 유저, 메세지 영역들 분리해서 출력하는 걸로 수정
6. 다른 채널로 변경
- [x] 변경할 채널 이름 선택(전체 채널 중 본인 채널 제외한 뷰 제공)
- [x] 채널 변경(+메시지 불러오기)
...
# 미완성 및 개선해야할 부분
- [ ] DI 구현: 싱글톤에서 구현하기 위해 팩토리 메서드 사용예정
- [ ] 각 엔티티 CRUD로직 컨트롤러구현
- [ ] OutputView - createUserName에서 id 비교하는 부분 equals로 수정
- [x] Application 사용자 입력 처리부분 Enum으로 수정
- [ ] JCFChannelService 스트림처리 부분 개선필요
- [ ] InputView Scanner 자원할당 방식에 문제 없는지 확인필요
- [ ] 도메인별 id 객체로 변경
  - 도메인 모델의 Id가 UUID에 의존하지 않도록 수정
- [ ] 전체적인 예외처리(커스템 예외 만들기)
  - [ ] 예외 발생시 입력을 다시 받거나 따로 처리해주기
- [ ] 응용서비스에 들어있는 기능 도메인 서비스로 옮길 수 있나 생각해보기