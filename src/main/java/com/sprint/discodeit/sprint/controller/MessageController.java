...
public class MessageController {
    ...
    public ResponseEntity<Message> sendMessage(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(messageService.create(channelId, requestDto));
    }

    @Operation(summary = "메시지 수정", description = "메시지 내용을 수정합니다.")
    @PutMapping("/{messageId}")
    public ResponseEntity<Message> updateMessage(
            @Parameter(description = "수정할 메시지의 UUID") @PathVariable Long messageId,
    ...
    public ResponseEntity<Void> deleteMessage(
            @Parameter(description = "삭제할 메시지의 UUID") @PathVariable Long messageId) {
