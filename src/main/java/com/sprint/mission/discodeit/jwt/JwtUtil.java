...
public class JwtUtil {
  ...
  protected void init() {
    ...
    try {
      // 일반 문자열을 UTF-8 바이트 배열로 변환
      byte[] keyBytes = secretKeyString.getBytes(StandardCharsets.UTF_8); // <<< 수정된 부분
      this.key = Keys.hmacShaKeyFor(keyBytes);
      log.info(
          "JWT Secret Key initialized directly from string bytes (assuming plain text secret).");
      if (keyBytes.length * 8 < 256) {
        log.warn(
            "The configured JWT secret key's size is less than 256 bits (which is low for HS256)...");
      }
    } catch (Exception e) { // 포괄적인 예외 처리
      log.error("FATAL: Error initializing JWT key from string bytes!", e);
      // 실제 운영 환경에서는 더 구체적인 예외 처리나 복구 로직이 필요할 수 있습니다.
      throw new IllegalStateException("Error initializing JWT key from string bytes.", e);
    }
  }
