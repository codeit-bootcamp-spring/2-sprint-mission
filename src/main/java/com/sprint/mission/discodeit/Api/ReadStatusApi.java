...
public interface ReadStatusApi {
   ...
   * @return 이미 읽음 상태가 존재함 (status code 400) or Message 읽음 상태가 성공적으로 생성됨 (status code 201) or
   * Channel 또는 User를 찾을 수 없음 (status code 404)
   */
  @Operation(
      operationId = "create1",
      ...
      responses = {
          ...
          @ApiResponse(responseCode = "201", description = "Message 읽음 상태가 성공적으로 생성됨", content = {
              @Content(mediaType = "*/*", schema = @Schema(implementation = ReadStatusDto.class))
          }),
  ...
  default ResponseEntity<ReadStatusDto> create1(
      @Parameter(name = "ReadStatusCreateRequest", description = "", required = true) @Valid @RequestBody ReadStatusCreateRequest readStatusCreateRequest
  ) {
    getRequest().ifPresent(request -> {
      for (MediaType mediaType : MediaType.parseMediaTypes(request.getHeader("Accept"))) {
        if (mediaType.isCompatibleWith(MediaType.valueOf("*/*"))) {
          String exampleString = "{ \"lastReadAt\" : \"\", \"id\" : \"\", \"userId\" : \"\", \"channelId\" : \"\" }";
   ...
   * @return Message 읽음 상태가 성공적으로 수정됨 (status code 200) or Message 읽음 상태를 찾을 수 없음 (status code 404)
   */
  @Operation(
      operationId = "update1",
  ...
  default ResponseEntity<ReadStatusDto> update1(
