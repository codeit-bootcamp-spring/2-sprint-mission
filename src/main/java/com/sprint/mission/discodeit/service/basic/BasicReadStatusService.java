...
public class BasicReadStatusService implements ReadStatusService {
    ...
    @Transactional
    @Override
    public ReadStatusDto create(ReadStatusCreateRequest request, CustomUserDetails principal) {
    ...
    @Transactional
    @Override
    public ReadStatusDto update(UUID readStatusId, ReadStatusUpdateRequest request, CustomUserDetails principal) {
        log.debug("읽음 상태 수정 시작: id={}, newLastReadAt={}", readStatusId, request.newLastReadAt());

        ReadStatus readStatus = readStatusRepository.findById(readStatusId)
                .orElseThrow(() -> ReadStatusNotFoundException.withId(readStatusId));

        UUID targetUserId = readStatus.getUser().getId();
        checkPermission(targetUserId, principal);
    ...
    private void checkPermission(UUID targetUserId, CustomUserDetails principal) {
        boolean isAdmin = principal.getAuthorities().stream()
                .anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN"));
        boolean isSelf = principal.getUser().getId().equals(targetUserId);
