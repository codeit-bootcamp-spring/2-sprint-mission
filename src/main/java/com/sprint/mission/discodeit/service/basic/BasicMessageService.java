...
public class BasicMessageService implements MessageService {
    ...
    @Transactional
    @Override
    public MessageDto update(UUID messageId, MessageUpdateRequest request, CustomUserDetails principal) {

        log.debug("메시지 수정 시작: id={}, request={}", messageId, request);

        checkUpdatePermission(messageId, principal);
    ...
    @Transactional
    @Override
    public void delete(UUID messageId, CustomUserDetails principal) {

        log.debug("메시지 삭제 시작: id={}", messageId);

        checkDeletePermission(messageId, principal);
    ...
    private void checkUpdatePermission(UUID messageId, CustomUserDetails principal) {
        Message message = messageRepository.findById(messageId)
                .orElseThrow(() -> MessageNotFoundException.withId(messageId));

        boolean isAuthor = principal.getUser().getId().equals(message.getAuthor().getId());
        if (!isAuthor) {
            throw new AccessDeniedException("메시지 수정 권한이 없습니다.");
        }
    }

    private void checkDeletePermission(UUID messageId, CustomUserDetails principal) {
        Message message = messageRepository.findById(messageId)
                .orElseThrow(() -> MessageNotFoundException.withId(messageId));
