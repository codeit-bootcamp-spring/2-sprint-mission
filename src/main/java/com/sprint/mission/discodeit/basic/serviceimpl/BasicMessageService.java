...
public class BasicMessageService implements MessageService {
    ...
    @Override
    public MessageDto.Response updateMessage(UUID messageId, MessageDto.Update messageUpdateDTO, UUID uuid) {
        Message message = messageRepository.findById(messageId)
                .orElseThrow(() -> new ResourceNotFoundException("Message", "id", messageId));

        if (messageUpdateDTO.getMessage() != null && !messageUpdateDTO.getMessage().equals(message.getMessage())) {
        ...
        if (operation != null && binaryContentIds != null && !binaryContentIds.isEmpty()) {
            ...
            switch (operation) {
                case add:
                    ...
                    boolean changed = message.getAttachmentIds().addAll(binaryContentIds);
                    break;
                case remove:
                    ...
                    if (!deleteList.isEmpty()) {
                            ...
                            message.getAttachmentIds().removeAll(deleteList);
                    }
                    break;
                default:
