...
import com.sprint.mission.discodeit.exception.status.read.ReadStatusError;
...
public class ReadStatusErrorAdvice {
  ...
  public ResponseEntity<ErrorResponse> handleUserAlreadyExistsError(ReadStatusAlreadyExistsError error) {
    return ResponseEntity.badRequest().body(new ErrorResponse(error.getMessage()));
  }
