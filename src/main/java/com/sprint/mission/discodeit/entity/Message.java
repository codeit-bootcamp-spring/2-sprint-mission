...
public class Message implements Serializable {
  ...
  public Message(UUID authorId, UUID channelId, String content, List<UUID> attachmentIds) {
    validateMessage(authorId, channelId, content);
   ...
   * Validation check
   *******************************/
  private void validateMessage(UUID authorId, UUID channelId, String content) {
    // 1. null check
    if (content == null) {
      throw new IllegalArgumentException("메시지 내용이 null입니다.");
    }
    if (authorId == null) {
      throw new IllegalArgumentException("메시지 작성자가 없습니다.");
    }
    if (channelId == null) {
