...
public class JavaApplicationCodeitBase {
    static void userCRUDTest(UserService userService) {
        ...
        // 수정
        User updatedUser = userService.update(user.getId(), "king", "kang@naver.com", "woody5678");
...
//        List<Channel> foundChannelsAfterDelete = channelService.getAllChannel();
...
//        UUID authorId = UUID.randomUUID();
        String uuid1 = "922370f5-f7ac-46f4-8bb5-503a0986f1ab";
        UUID channelId = UUID.fromString(uuid1);
        String uuid2 = "b51ac025-5207-48e2-81e2-95fff9625e8b";
        UUID authorId = UUID.fromString(uuid2);
        Message message = messageService.create("안녕하세요.", channelId, authorId);
        System.out.println("메시지 생성: " + message.getId());
        // 조회
        Message foundMessage = messageService.getMessage(message.getId());
        System.out.println("메시지 조회(단건): " + foundMessage.getId());
        List<Message> foundMessages = messageService.getAllMessage();
        System.out.println("메시지 조회(다건): " + foundMessages.size());
        // 수정
        Message updatedMessage = messageService.update(message.getId(), "반갑습니다.");
        System.out.println("메시지 수정: " + updatedMessage.getMessage());
        // 삭재
        messageService.delete(message.getId());
        List<Message> foundMessagesAfterDelete = messageService.getAllMessage();
        System.out.println("메시지 삭제: " + foundMessagesAfterDelete.size());
    }

    static User setupUser(UserService userService) {
        User user = userService.create("JO", "woody@codeit.com", "woody1234");
        return user;
    }

    static Channel setupChannel(ChannelService channelService) {
        Channel channel = channelService.create("1", "공지 채널입니다.");
        return channel;
    }
