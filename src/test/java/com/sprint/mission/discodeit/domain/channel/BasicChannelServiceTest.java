...
class BasicChannelServiceTest extends IntegrationTestSupport {
  ...
  void getById_Public() {
    // given
    ChannelResult savedChannel = channelService.createPublic(
  ...
  void getById_Private() {
    // given
    User savedUser = userRepository.save(new User("", "", "", null));
    ChannelResult savedChannel = channelService.createPrivate(
        new PrivateChannelCreateRequest(List.of(savedUser.getId())));
    ...
    Assertions.assertThat(channel)
        .extracting(ChannelResult::id, ChannelResult::type,
            c -> c.participants().stream().map(UserResult::id).toList())
        .containsExactlyInAnyOrder(savedChannel.id(), savedChannel.type(),
            List.of(savedUser.getId()));
  }
  ...
  void getAllByUserId() {
    ...
    ChannelResult aPrivate = channelService.createPrivate(
        new PrivateChannelCreateRequest(List.of(savedUser.getId())));
            ...
            .extracting(ChannelResult::id, ChannelResult::type,
                channel -> {
                  ...
                  return participants == null ? null
                      : participants.stream().map(UserResult::id).toList();
                })
  ...
  void updatePublic() {
    ...
    // when
    ChannelResult updatedPublic = channelService.updatePublic(savedChannel.getId(),
  ...
  void updatePublic_Exception() {
    ...
    Assertions.assertThatThrownBy(
            () -> channelService.updatePublic(UUID.randomUUID(), publicChannelUpdateRequest))
        .isInstanceOf(ChannelNotFoundException.class);
  }
  ...
  void updatePrivate_Exception() {
    ...
    // when & then
    Assertions.assertThatThrownBy(
            () -> channelService.updatePublic(savedChannel.getId(), publicChannelUpdateRequest))
  ...
  void delete() {
    ...
    // then
    assertAll(
        () -> Assertions.assertThat(channelRepository.findById(savedChannel.getId()))
            .isNotPresent(),
        () -> Assertions.assertThat(messageRepository.findByChannel_Id(savedChannel.getId()))
            .hasSize(0)
    );
  }
  ...
  void delete_Private() {
    ...
    messageRepository.save(
        new Message(channelRepository.findById(aPrivate.id()).get(), savedUser, "", List.of()));
