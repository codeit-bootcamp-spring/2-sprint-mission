...
class ChannelControllerTest {
    ...
    void setUp() {
        ...
        setUpUser = userService.register(new UserCreationRequest(LOGIN_USER.getName(), LOGIN_USER.getEmail(), LOGIN_USER.getPassword()), null);
    }

    @DisplayName("Public 채널 생성")
    @Test
    void createPublicChannel() {
        ChannelResult channel = channelController.createPublicChannel(new ChannelCreateRequest("Public", setUpUser.id())).getBody();
    ...
    void createPrivateChannel() {
        UserResult otherUser = userService.register(new UserCreationRequest(OTHER_USER.getName(), OTHER_USER.getEmail(), OTHER_USER.getPassword()), null);
    ...
    void findAll() {
        ...
        ChannelResult userPublicChannel = channelController.createPublicChannel(new ChannelCreateRequest("Public", setUpUser.id())).getBody();
        UserResult otherUser = userService.register(new UserCreationRequest(OTHER_USER.getName(), OTHER_USER.getEmail(), OTHER_USER.getPassword()), null);
        createPrivateChannel(otherUser, new ArrayList<>());


        List<UUID> channelIds = channelController.getAllByUserId(setUpUser.id())
                .getBody()
                .stream()
    ...
    void findByIdPrivateChannel_MemberTogether() {
        ...
        ChannelResult privateChannel = channelController.addPrivateChannelMember(new PrivateChannelAddMemeberRequest(channel.id(), OTHER_USER.getEmail())).getBody();
    ...
    private ChannelResult createPrivateChannel(UserResult loginUser, List<UUID> memberIds) {
        return channelController.createPrivateChannel(new PrivateChannelCreationRequest(CHANNEL_NAME, loginUser.id(), memberIds)).getBody();
    }
}