...
class BasicMessageServiceTest {
    ...
    private static final String UPDATED_MESSAGE_CONTENT = "반갑습니다";
    ...
    void createTest() {
        ...
        MessageResult expectedResult = new MessageResult(null, null, null,
                UserResult.fromEntity(user, false), MESSAGE_CONTENT, UUID.randomUUID(), List.of());

        BDDMockito.given(channelRepository.findById(any()))
        ...
        // when
        MessageResult messageResult = messageService.create(messageCreateRequest, List.of());
    ...
    @DisplayName("메세지 내용 수정시도했지만 해당 메세지가 없다면, 메세지 내용을 수정한다")
    @Test
    ...
    void deleteTest() {
        ...
        // then
        BDDMockito.verify(binaryContentStorageService)
                .deleteBinaryContentsBatch(any());
        BDDMockito.verify(messageRepository)
                .deleteById(any());
    }
